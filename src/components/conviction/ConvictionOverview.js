...
const ConvictionOverview = ({ issue, convictionList }) => {
    ...
    const generateConvictionDataSets = stakeHistory =>
        convictionList.map((user, userIndex) => {
          ...
          user.stakes[stakeIndex].time <= time
                    ...
                    const action = user.stakes[stakeIndex];
                    stakeIndex++;
    ...
    const calculateConvictionPlotData = () => {
        ...
        const convictionDataSets = generateConvictionDataSets(stakeHistory);
        // add a dataset with the total conviction
        const totalConvictionData = [];
        const triggerValues = [];
        let convictionThresholdPassed = false;
        for (let time = 0; time < globalParameters.totalTime; time++) {
            const totalConviction = convictionDataSets.reduce(
